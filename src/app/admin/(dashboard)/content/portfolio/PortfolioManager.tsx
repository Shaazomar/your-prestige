...
import { listProjects, softDeleteProject, restoreProject, type ProjectRow } from "./actions";

export function PortfolioManager({ permissions }: { permissions: { create: boolean; edit: boolean; delete: boolean } }) {
  const list = useAdminList<ProjectRow>(listProjects);
  ...
  const columns: Column<ProjectRow>[] = [
    {
      ...
      render: (row) => (
        <div>
          ...
          <p className="text-xs text-white/35">{row.location || `/${row.slug}`}</p>
        </div>
      ),
    },
    { key: "type", label: "Type", render: (row) => <span className="rounded-full bg-white/8 px-2.5 py-1 text-xs text-white/60">{row.type}</span> },
    { key: "builder", label: "Builder", render: (row) => <span className="text-white/60">{row.builder ?? "—"}</span> },
    { key: "year", label: "Year", sortable: true, render: (row) => <span className="text-white/60">{row.year ?? "—"}</span> },
  ...
  return (
    <>
      <AdminDataTable
        ...
        emptyMessage={list.trash ? "Trash is empty." : "No projects yet — showcase your first build."}
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title={editing ? "Edit Project" : "New Project"} wide>
        <ProjectForm project={editing} onSuccess={onFormSuccess} />
      </Drawer>

      <ConfirmDialog
        open={!!deleting}
        title="Move to trash?"
        description={`"${deleting?.title}" will be moved to trash. You can restore it anytime.`}
